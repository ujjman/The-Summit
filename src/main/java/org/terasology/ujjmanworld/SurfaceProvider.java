...
public class SurfaceProvider implements FacetProvider {
    ...
    public void setSeed(long seed) {
        surfaceNoise = new SubSampledNoise(new BrownianNoise(new SimplexNoise(seed + 5), 5), new Vector2f(0.02f, 0.01f), 1);
    }
    ...
    public void process(GeneratingRegion region) {
        ...
        for (BaseVector2i position : processRegion.contents()) {
            facet.setWorld(position, surfaceNoise.noise(position.x(), position.y()) * 100); // increasing world height 100 times 
        }
